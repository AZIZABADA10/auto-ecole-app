    ...
    <div class="max-w-3xl mx-auto bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        ...
        <form action="{{ isset($user) ? route('users.update', $user) : route('users.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    @if(request('type') === 'staff' || (isset($user) && !$user->isCandidat()))
                        <label for="role_id" class="block text-sm font-medium text-gray-700 mb-2">Rôle Staff *</label>
                        <select name="role_id" id="role_id" class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-emerald-500 focus:border-emerald-500 bg-gray-50 focus:bg-white" required>
                            @foreach($roles as $role)
                                @if(in_array($role->name, ['assistante', 'moniteur', 'admin']))
            ...
            <div class="pt-6 border-t border-gray-100 flex justify-end gap-3 mt-8">
                <a href="{{ route('users.index') }}" class="px-6 py-2.5 bg-white border border-gray-300 text-gray-700 rounded-xl hover:bg-gray-50 transition shadow-sm font-medium">
                    Annuler
                </a>
